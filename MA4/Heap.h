...
/********************* Start Microassignment zone ********************/
    ...
    {
        if (index * 2 < _items.size())
        {
            int minChildIndex = index * 2;
            if (index * 2 + 1 < _items.size() && _items[index * 2] > _items[index * 2 + 1])
            {
                minChildIndex = index * 2 + 1;
            }
            if (_items[index] > _items[minChildIndex])
            {
                T t = _items[index];
                _items[index] = _items[minChildIndex];
                _items[minChildIndex] = t;
